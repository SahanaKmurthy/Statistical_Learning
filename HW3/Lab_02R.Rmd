...
p <- pnorm(q=5, mean=6, sd=4/sqrt(10))
p
```
```
...
```{r}
# FILL ME IN
p_lower <- pnorm(q=5, mean=6, sd=4/sqrt(10))
p_upper <- 1 - pnorm(q=5, mean=6, sd=4/sqrt(10))
p_twotail <- 2 * min(c(p_lower, p_upper))
p_twotail
```
...
```{r}
# FILL ME IN
p <- pnorm(q=5, mean=6, sd=4/sqrt(100))
p
```
```
FILL ME IN WITH TEXT
...
```{r}
# FILL ME IN
x.bar.rr <- qnorm(p=0.05, mean=6, sd=4/sqrt(10))

p <- pnorm(q=x.bar.rr, mean=5.5, sd=4/sqrt(10))
...
Since the p-value is greater than 0.05, I will fail to reject the null hypothesis.
```
...
# FILL ME IN
sd = sd(x)
p <- 1 - pnorm(q=7.5, mean=9, sd/sqrt(15))
sd
p
...
```
FILL ME IN WITH TEXT
Since I am getting a p value of 0.96, I would fail to reject my null hypothesis.
```
...
write down, but
\it turns out that's OK, since we can write down that 
...
stat <- (n-1) * sample_variance / sigma_squared
pchisq(stat, df = n-1)
```
...
# FILL ME IN
varTest(x, alternative = "less" , sigma.squared = 25)
```
...
x_bar <- mean(x)
p <- 1 - pgamma(x_bar, shape = n, scale = 1/n)
p
...
The histogram does not show the bell curve of the normal distribution. I would like to say that we could do more testing before concluding that the p-values are distributed uniformly.
```
...
Since p value is greater 0.05, we fail to reject the hypothesis.
```
