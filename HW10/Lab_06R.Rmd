...
pc with 5 components explains 90% of the variability, so I am choosing 5 PC's
```
...
```{r}
# FILL ME IN WITH CODE
pca_df_12 <- data.frame(PC1 = pca.out$x[,1] , PC2 = pca.out$x[,2])
ggplot(pca_df_12 , mapping = aes(x = PC1 , y = PC2 , color = response)) +
         geom_point(size = 1) +
         labs(
           x = ("PC1") ,
           y = ("PC2") ,
...
```{r}
# FILL ME IN WITH CODE
pca_df <- data.frame(PC2 = pca.out$x[,2] , PC3 = pca.out$x[,3])
ggplot(pca_df , mapping = aes(x = PC2 , y = PC3 , color = response)) +
         geom_point(size = 3) +
         labs(
           x = ("PC2") ,
           y = ("PC3") ,
...
pca_df <- data.frame(PC3 = pca.out$x[,3] , PC4 = pca.out$x[,4])
ggplot(pca_df , mapping = aes(x = PC3 , y = PC4 , color = response)) +
         geom_point(size = 3) +
         labs(
           x = ("PC3") ,
           y = ("PC4") ,
...
```{r}
# FILL ME IN WITH CODE
PC1_df <- data.frame(PC1 = pca.out$x[,1], response = response)
ggplot(PC1_df , aes(x= response , y = PC1, fill = response)) +
  geom_boxplot() +
  labs(
    x = "Response" , y = "Principal Component 1"
  )
```
