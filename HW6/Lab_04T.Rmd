...
distance = sqrt((2-3)squared + (2-3)squared + (2-3)squared)
         = sqrt(1+1+1)
         = sqrt(3)
```
...
The ratio is around 21.61%. This means only a small portion of the variance is explained by the clustering. The clusters are not well separated.
```
...
```
FILL ME IN WITH TEXT
I think the optimal number of clusters is 2?
```
