...
The length of the dataframe returns the number of vectors (or number of colums) in the df
```
...
```
FILL ME IN WITH TEXT
The maximum magnitudes of some of the columns are 99, which might be unusual? since a magnitude of 20 means the object is 100 million times fainter than a star like Vega, I think that the magnitude of 99 means that the object is 495 million times fainter than the Vega star.
```
...
the means of the gr.bright is signigicantly higher than the mean of gr.faint. And the p-value is very close to 0, so we can reject the null hypothesis.
```
