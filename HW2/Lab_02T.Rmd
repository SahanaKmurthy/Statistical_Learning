...
z <- sample(x, size = 100, replace = TRUE)
sum(z < 5)
length(x[z])
length(z < 5) / length(z)
...
# FILL ME IN
mu = mean(x)
f <- function(mu, x.bar, s, q)
{
  pnorm(x.bar, mu, s) -q
}
uniroot(f, interval = c(-100,100), x.bar = mu , s=0.2, q= 1-alpha/2)$root
uniroot(f, interval = c(-100,100), x.bar = mu , s=0.2, q= alpha/2)$root
...
uniroot(f, interval = c(-100,100), x.bar = mu , s=0.2, q= alpha/2)$root
uniroot(f, interval = c(-100,100), x.bar = mu , s=0.2, q= alpha)$root
...
f <- function(x.sum, nlambda, s, q)
{
  ppois(x.sum, nlambda, s) -q
}
uniroot(f, interval = c(0,100), x.sum = sum(x) , s=0.2, q= 1-alpha/2)$root/10
uniroot(f, interval = c(0,100), x.sum = sum(x) , s=0.2, q= alpha/2)$root/10
...
x <- sample(1:100000,100000, replace = TRUE)
length(unique(x)) / 100000
```
